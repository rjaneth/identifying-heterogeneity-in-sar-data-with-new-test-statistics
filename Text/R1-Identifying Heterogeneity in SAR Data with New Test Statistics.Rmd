...
In order to clarify the limit behavior when \(\alpha \to -\infty\) in Equation \eqref{E:E-GIO}, we show in Appendix&nbsp;\ref{app:1} that, for \(L=1\) and in the general case, the expression simplifies and the terms involving \(\alpha\) cancel out. Thus, the entropy of \(G_I^0\) converges to the entropy \(\Gamma_{\text{SAR}}\).
...
colnames(SAR_data) <- c("\\textbf{Site}", "\\textbf{Mission}", "\\textbf{Mode}", "\\textbf{Band}", "\\textbf{Polarization}", "\\textbf{Size}",  "$\\bm{L}$", "\\textbf{Resolution Rg/Az [m]} ", "\\textbf{Adquisition Date}")
...
The three statistical tests are applied to the SAR images using $7\times 7$ local sliding windows (this window size is commonly used in practical scenarios, which strikes a balance between capturing detailed local information and ensuring a sufficient number of data points for solid analysis), as illustrated in Figures&nbsp;\ref{fig:test_Rotterdam}, \ref{fig:real_images_test_Mexico}, \ref{fig:test_lake} and&nbsp;\ref{fig:real_images_test_Illinois}.
...
\begin{align*}
	A=\ln\frac{(-1-\alpha)\cdot\Gamma(-\alpha)}{\Gamma(1-\alpha)}=\ln\frac{(-1-\alpha)\cdot\Gamma(-\alpha)}{-\alpha\Gamma(-\alpha)}=\ln\frac{-1-\alpha}{-\alpha}=\ln\left(1+\frac{1}{\alpha}\right).
\end{align*}
...
Substituting the simplified expressions of \(A\) and \(B\) into \eqref{E:lim} and taking the limit, we obtain:
\begin{align*}
...
\[
-\ln\Gamma(L-\alpha) \sim -\frac{1}{2}\ln(2\pi(L-\alpha))-(L-\alpha)\ln(L-\alpha)+(L-\alpha).
\]
\[
...
Then, substituting  these terms in \eqref{E:lim1} and applying limit we obtain:

\begin{multline*}
  \lim_{\alpha\to-\infty}\bigg[-\frac{1}{2}\ln(2\pi(L-\alpha))-L\ln(L-\alpha)+\alpha\ln(L-\alpha)+L-\alpha+ L\ln(L-\alpha) -\alpha\ln(L-\alpha)\\- \frac{1}{2}
