...
<!-- ACF Define the estimators and state their main properties -->
...
We analyzed the performance of these estimators with a Monte Carlo study: \(500\) samples from the \(G_I^0\) law of size
\(n\in\left\{9, 25, 49, 81, 121\right\}\) and different mean, number of looks and roughness. 
We present the results with
\(\mu\in\left\{1, 10\right\}\), \(\alpha=-10\) and \(L=5\). 
The results are consistent with the other situations. 
We used $B=100$ bootstrap samples and the heuristic spacing
...
We explore a robust alternative to CV, as described by \citet{Ospina2019}, which incorporates the ratio between the mean absolute deviation from the median (MnAD) and the median, two well-known robust measures of
...
Results with \(p\) values greater than \(0.05\) do not indicate a violation of the normality assumption. A low variance as well as a skewness and an excessive kurtosis of almost zero indicate limited dispersion, symmetry and a light tail.
