...
# ACF Which is "our proposal" and which are the "competitors"? # ok
keywords: |
...
supplementary: |
 ...
 \url{https://github.com/rjaneth/identifying-heterogeneity-in-sar-data-with-new-test-statistics} (accessed on 24 April 2024).
# abbreviations:
#   - short: MDPI
...
The coefficient of variation is the basic measure of heterogeneity in SAR data&nbsp;\cite{oliverquegan98}.
We study two ways of estimating the coefficient of variation.

The other parameter we study is the Shannon entropy.
Different models for SAR data have different entropy values, but this fundamental quantity can be also estimated in a model-agnostic way.
We exploit this property and design a bootstrap-improved non-parametric estimator for the Shannon entropy.

We devise test statistics based on these three estimators: the classical coefficient of variation, a robust version, and the Shannon entropy estimator.
...
Through extensive experiments with simulated and SAR data, we demonstrate the superiority of our proposed method over existing approaches.
...
\begin{align}
    E_{\mathcal{G}_I^0}\left(Z^r\right)=\left(\frac{\gamma}{L}\right)^r\frac{\Gamma(-\alpha-r)}{\Gamma(-\alpha)}\cdot\frac{\Gamma(L+r)}{\Gamma(L)}, \quad \alpha <-r. 
    \label{E:rmom}
\end{align}
 
Although the \(\mathcal{G}_I^0\) distribution is defined by the parameters \(\alpha\) and \(\gamma\), in the SAR literature&nbsp;\cite{Nascimento2010} the texture \(\alpha\) and the mean \(\mu\) are usually used. 
The expected value \(\mu\) using the expression in&nbsp;\eqref{E:rmom} is
\begin{equation}
    \mu=\left(\frac{\gamma}{L}\right)\frac{\Gamma(-\alpha-1)}{\Gamma(-\alpha)}\cdot\frac{\Gamma(L+1)}{\gamma(L)}=-\frac{\gamma}{\alpha+1},
\end{equation} 
and we reparametrize&nbsp;\eqref{E:gi01} with \(\mu\) obtaining:
\begin{equation}
...
Several authors&nbsp;\cite{vasicek1976test, Wieczorkowski1999,correa1995new,AlOmari2019} have studied the problem of the non-parametric estimation of $H(Z)$.
...
\label{E:Vas}
	...
	\end{equation*}
where $m<n/2$ is a positive integer, $Z_{(i+m)}-Z_{(i-m)}$ is the $m$-spacing and $Z_{(1)}\leq Z_{(2)}\leq\ldots\leq Z_{(n)}$ are the order statistics, $Z_{(i)}= Z_{(1)}$ if $i<1$, $Z_{(i)}= Z_{(n)}$ if $i>n$.
...
The results are consistent with the other situations. 
...
colnames(reshaped_results) <- c("$\\mu$", "$n$",  "$\\widehat{H}_{\\text{C}}$", "$\\widehat{H}_{\\text{E}}$","$\\widehat{H}_{\\text{AO}}$",  "$\\widetilde{H}_{\\text{C}}$", "$\\widetilde{H}_{\\text{E}}$","$\\widetilde{H}_{\\text{AO}}$",   "$\\widehat{H}_{\\text{C}}$", "$\\widehat{H}_{\\text{E}}$","$\\widehat{H}_{\\text{AO}}$", "$\\widetilde{H}_{\\text{C}}$", "$\\widetilde{H}_{\\text{E}}$","$\\widetilde{H}_{\\text{AO}}$")
...
We explore a robust alternative to CV, as describedby&nbsp;\cite{Ospina2019}, which incorporates the ratio between the mean absolute deviation from the median (MnAD) and the median, two well-known robust measures of
...
If the data represent fully-developed speckle, the density should center around zero, i.e., $S(\bm{Z};L_0)\approx 0$. Otherwise, under the alternative hypothesis, the empirical distribution would shift from zero, suggesting significant differences and implying the presence of heterogeneous clutter. 
...
Figure&nbsp;\ref{fig:Plot_density} shows the empirical test statistic densities for different sample sizes using \(\widetilde{H}_{\text{AO}}\) estimator, and Table&nbsp;\ref{tab:table_stat_combined} summarizes the main descriptive statistics, including mean, SD, variance (Var), skewness (SK), excessive kurtosis (EK) and Anderson--Darling \(p\) values for normality. 
Results with \(p\) values greater than \(0.05\) do not indicate a violation of the normality assumption.
A low variance as well as a skewness and an excessive kurtosis of almost zero indicate limited dispersion, symmetry and a light tail.
...
table_result_combined <- knitr::kable(
  ...
  caption = "Descriptive analysis of $S(\\bm Z; L)$ with $L\\in\\left\\{3,5, 8,11\\right\\}$ and $\\mu=1$.",
  format = "latex",
  booktabs = TRUE,
  align = "lrrrrrrr",
  escape = FALSE,
...
After checking the normality of the test statistics, we examined the possibilities of the proposed test in terms of size and power.
...
\SI{10}{\percent}. 
In terms of size, \(1000\) simulations were used for different sample sizes from the $\Gamma_{\text{SAR}}$ distribution, with different values of $L$,  $\mu=1$ and \(\widetilde{H}_{\text{AO}}\) estimator. 
In all cases, the nominal level was achieved. 
We assessed the test power using \(1000\) simulations for different sample sizes from the $G_I^0$ distribution, with  $\mu=1$, $\alpha=-2$, and \(\widetilde{H}_{\text{AO}}\) estimator.
...
Figure&nbsp;\ref{fig:sim_Phantom} shows the phantom used for a simulated image with dimensions of $500\times 500$ pixels that consists of four smaller phantoms, each of which exhibits differences between homogeneous and heterogeneous regions.  
The regions are formed by independent draws from the $G^0_I$ distribution, with $L = 5$ and varying $\alpha$. 
...
As the value decreases, it indicates regions with moderate texture (in $\left[-6,-3\right]$), related to forest zones, while values below $-6$ correspond to textureless regions, such as pasture, agricultural fields, and water bodies&nbsp;\cite{Neto2023}.



```{r sim_Phantom, echo=FALSE, message=FALSE, warning=FALSE, out.width="40%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Simulated image: Light regions in upper/lower left phantoms show homogeneity ($\\alpha=-10$), while dark ones suggest heterogeneity ($\\alpha=-2$). Conversely, in upper/lower right phantoms, dark regions mean homogeneity ($\\alpha=-9$), light ones mean heterogeneity ($\\alpha=-3$). ', fig.width = 4, fig.height = 4}
knitr::include_graphics("../Figures/PNG/Phantom.png")

```

The three test statistics (entropy-based, ${\text{CV}}$, and ${\text{CV}_{\text{MnAD}}}$) are applied to the simulated image using local sliding windows of size $7\times 7$, as shown in Figures&nbsp;\ref{fig:sim_results}(a)--(c). 
For the first test we considered the \(\widetilde{H}_{\text{AO}}\)  estimator due to its superior performance in simulations and shorter processing time compared to other entropy estimators. 

```{r sim_results, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results of applying the test statistics: (a)  Entropy-based test; (b)  CV test; (c)   $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c('', '', ''), out.width = c( "40mm","40mm", "40mm"),  fig.ncol=3, fig.width = 40, fig.height = 30}

knitr::include_graphics("../Figures/PNG/test-entropy-Phantom_AO_7W_L5_100b.png")
knitr::include_graphics("../Figures/PNG/cv_values_Phantom.png")
knitr::include_graphics("../Figures/PNG/mnad_Phantom.png")
```

The resulting $p$-values for each test  are shown in Figures&nbsp;\ref{fig:sim_SAR_Images}(a)--(c) and Figures&nbsp;\ref{fig:sim_SAR_Images_p05}(a)--(c), respectively. 
The new images reveal that a lighter  background indicates homogeneity, while darker backgrounds represent heterogeneous areas.

```{r sim_SAR_Images, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Map of $p$-values: (a) Entropy-based test; (b)  CV test; (c)  $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c('', '', ''), out.width = c( "40mm","40mm", "40mm"),  fig.ncol=3, fig.width = 40, fig.height = 30}

knitr::include_graphics("../Figures/PNG/H_pvalue_Phantom_AO_7x7_L5_100b.png")
knitr::include_graphics("../Figures/PNG/cv_p_values_Phantom.png")
knitr::include_graphics("../Figures/PNG/mnad_p_values_Phantom.png")
```
```{r sim_SAR_Images_p05, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results for a threshold of $0.05$ of the $p$-value: (a) Entropy-based test; (b) CV test; (c) $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
knitr::include_graphics("../Figures/PNG/H_005_pvalues_Phantom_AO_7x7.png")
knitr::include_graphics("../Figures/PNG/nominal_level_005_pvalues-Phantom.png")
knitr::include_graphics("../Figures/PNG/mnad_005_pvalues-Phantom.png")
...
We evaluated the proposed test statistics using three real SAR images. 
For our analysis, we used images of the coast of Jalisco, Mexico, and of Illinois, USA. 
These images were acquired by the Sentinel-1B satellite operating in C-band, with VV polarization, intensity format, \(L=18\) and \(L=36\)  nominal looks.
...
In Figures &nbsp;\ref{fig:Mexico_crops_0.05}, \ref{fig:lake_0.05} and&nbsp;\ref{fig:Illinois_crops_0.05}, the maps of $p$-values represent the decisions at a $5\%$ significance level. 
...
```{r real_SAR_Images_coe, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Real SAR images: (a) Coast of Jalisco; (b) Illinois-Region 1; (c) Illinois-Region 2. ',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm", "40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```{r real_images_test_Mexico, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results of applying the test statistics, Coast of Jalisco: (a)  Entropy-based test using $\\widetilde{H}_{\\text{AO}}$ estimator; (b)  CV test; (c)   $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c('', '', ''), out.width = c( "40mm","40mm", "40mm"),  fig.ncol=3, fig.width = 40, fig.height = 30}
...
```{r Mexico_pvalue, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Map of $p$-values, Coast of Jalisco: (a) Entropy-based test; (b)  CV test; (c)  $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```{r Mexico_crops_0.05, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results for a threshold of $0.05$ of the $p$-value for the Coast of Jalisco. (a) Entropy-based test; (b) CV test; (c) $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```{r test_lake, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results of applying the test statistics, Illinois-Region 1: (a)  Entropy-based test using $\\widetilde{H}_{\\text{AO}}$ estimator; (b)  CV test; (c)   $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c('', '', ''), out.width = c( "40mm","40mm", "40mm"),  fig.ncol=3, fig.width = 40, fig.height = 30}
...
```{r lake_pvalue, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Map of $p$-values, Illinois-Region 1: (a) Entropy-based test; (b)  CV test; (c)  $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```{r lake_0.05, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results for a threshold of $0.05$ of the $p$-value, Illinois-Region 1. (a) Entropy-based test; (b) CV test; (c) $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```{r real_images_test_Illinois, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results of applying the test statistics, Illinois-Region 2: (a)  Entropy-based test using $\\widetilde{H}_{\\text{AO}}$ estimator; (b)  CV test; (c)   $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c('', '', ''), out.width = c( "40mm","40mm", "40mm"),  fig.ncol=3, fig.width = 40, fig.height = 30}
...
```{r Illinois_crops_pvalue, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Map of $p$-values, Illinois-Region 2: (a) Entropy-based test; (b)  CV test; (c)  $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
```
```{r Illinois_crops_0.05, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results for a threshold of $0.05$ of the $p$-value, Illinois-Region 2. (a) Entropy-based test; (b) CV test; (c) $\\text{CV}_{\\text{MnAD}}$ test.',  fig.subcap=c(' ', ' ',' ' ), out.width = c("40mm", "40mm","40mm"),  fig.ncol=3, fig.width = 20, fig.height = 10}
...
Moreover, assuming $5\%$ as the threshold for the $p$ values, the dark region produced by the $\text{CV}$ test was lower than produced by the $\text{CV}_{\text{MnAD}}$ and entropy test.
