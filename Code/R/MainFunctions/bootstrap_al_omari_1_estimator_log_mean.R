...
bootstrap_al_omari_1_estimator_log_mean <-  function(x, B){
  ...
  for(b in 1:B) {
    ...
    while (same_values) {
      ...
      if (!all(bx == bx[1])) {
        same_values <- FALSE
        entropy_result <-  bootstrap_al_omari_1_estimator(bx, B) - log(mean(bx))
...
# result <- bootstrap_correa_estimator_log_mean(data, 10)
