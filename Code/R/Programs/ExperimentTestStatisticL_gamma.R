...
difference.betweeen.GammaSAR.GI0.TwoLook <- function(alpha) {
  return(
    -lgamma(2-alpha) +
      (2-alpha) * digamma(2-alpha) -
...
ggplot(TestStatistics, aes(x = `Test Statistics`, col = factor(`Sample Size`), linetype = factor(`Sample Size`))) +
  ...
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.TwoLook(-1.01), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.TwoLook(-1.01), y=15, parse=TRUE, label="alpha==-1.01", hjust=-0.1, vjust=-1, col="red") +
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.TwoLook(-1.1), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.TwoLook(-1.1), y=15, parse=TRUE, label="alpha==-1.1", hjust=1.1, vjust=-1, col="red") +
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.TwoLook(-1.5), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.TwoLook(-1.5), y=15, parse=TRUE, label="alpha==-1.5", hjust=1.1, vjust=-1, col="red") +
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.TwoLook(-300), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.TwoLook(-300), y=15, parse=TRUE, label="alpha==-300", hjust=-.1, vjust=-1, col="red") +
